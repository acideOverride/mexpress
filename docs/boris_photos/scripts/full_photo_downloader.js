/**
 * Full Photo Downloader for Boris Photos
 * 
 * This script downloads all photo variations including those that are only 
 * visible after selecting Finition and Taille options.
 * 
 * It extracts variation patterns from the VARIATIONS.md file and also uses
 * known variation patterns from AFTER_SELECTED_STATE.md.
 * 
 * Usage: node full_photo_downloader.js [collection_name] [photo_title]
 */
...
const CONFIG = {
  ...
  // Known variation identifier patterns based on filename patterns
  variationPatterns: {
    'dibond caisse americaine / plexi caisse americaine': [
      'caisse-americaine', 'caisse_americaine',
      'caisse-a', 'caisse_a',
      'caisse-noir', '_caisse-a-noir', 
      'americaine', '_américaine-noir', 
      'noir-3', '-3-', '_3_'
    ],
    'dibond cadre alu / plexi cadre alu': [
      'cadre-alu', 'cadre_alu',
      'cadre-noir', 'cadre_noir',
      'alu-noir', 'alu-arg',
      '-2-', '_2_'
    ],
    'dibond/plexi': [
      'dibond', 'plexi', 'boris-molinier'
    ],
    'original': [
      '.jpeg', 'original', '-1-', '_1_'
    ]
  },
  // Additional variation filenames that might be missed
  additionalVariationNames: [
    'Alu-noirPano.jpg',
    '_américaine-noir-Pano.jpg',
    'A-Marron-3-2.jpg',
    'Alu-Noir-3-2.jpg',
    '_Caisse-A-Noir-3-2.jpg',
    '_Alu-Arg-3-2.jpg'
  ]
};
  ...
  for (const title of titles) {
    ...
    if (photoChunks && photoChunks.length > 0) {
      ...
      if (uniqueUrls.length > 0) {
        // Try to identify special variation patterns based on the photo's page HTML
        const specialVariations = findSpecialVariations(photoChunks[0]);
        
        console.log(`  - "${title}": Found ${uniqueUrls.length} images (${dataSrcMatches.length} main, ${slideUrls.length} slides, ${specialVariations.length} special variations)`);
...
/**
 * Find any special variation patterns in the HTML that might be hidden
 * @param {string} html - HTML chunk for a single photo
 * @returns {Array} Array of special variation URLs
 */
function findSpecialVariations(html) {
  const specialVariations = [];
  
  // Look for the additional variation names that are known to exist
  for (const variationName of CONFIG.additionalVariationNames) {
    const pattern = new RegExp(`https://images\\.squarespace-cdn\\.com/content/[^"]*${variationName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[^"]*`, 'g');
    let match;
    
    while ((match = pattern.exec(html)) !== null) {
      // Remove any query parameters
      const cleanUrl = match[0].split('?')[0];
      if (!specialVariations.includes(cleanUrl)) {
        specialVariations.push(cleanUrl);
      }
    }
  }
  
  return specialVariations;
}
...
/**
 * Categorizes a URL based on its filename and known variation patterns
 * @param {string} url - The URL to categorize
 * @returns {string|null} The variation category or null if not found
 */
function categorizeVariation(url) {
  const filename = url.split('/').pop().split('?')[0].toLowerCase();
  
  for (const [variation, patterns] of Object.entries(CONFIG.variationPatterns)) {
 ...
 */
function matchVariationsToPhoto(photo, variations) {
  const result = {};
  const urls = [...photo.urls, ...(photo.specialVariations || [])];
  ...
  for (const photoUrl of urls) {
    ...
    // If no exact match, try to identify the variation based on filename patterns
    if (!matched) {
      const variation = categorizeVariation(photoUrl);
      
      if (variation && !result[variation]) {
        result[variation] = photoUrl;
        matched = true;
      }
    }
  }
  ...
  if (!result['original'] && urls.length > 0) {
    result['original'] = urls[0];
  }
...
async function downloadPhotoVariations(photo, variations, collectionName) {
  ...
  console.log(`- Found ${photo.urls.length} standard URLs and ${photo.specialVariations?.length || 0} special variations`);
  
  // Match photo URLs to variations
  const matchedVariations = matchVariationsToPhoto(photo, variations);
  ...
  for (const [finition, url] of Object.entries(matchedVariations)) {
    ...
    if (CONFIG.additionalVariationNames.some(pattern => filename.includes(pattern.split('.')[0]))) {
...
async function main() {
  try {
    console.log('🚀 Starting Full Photo Downloader');
