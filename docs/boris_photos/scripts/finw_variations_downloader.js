/**
 * Find Variations Downloader for Boris Photos
 * 
 * This script finds variation patterns from VARIATIONS.md across all photos
 * in the collection_ListGrid.md file and downloads them.
 * 
 * Usage: node finw_variations_downloader.js [collection_name]
 */
...
const CONFIG = {
  ...
  collectionFile: '/opt/mExpress/docs/boris_photos/__mocks__/current/collection_ListGrid.md',
...
/**
 * Extracts photos and their URLs from the collection file
 * @returns {Array} Array of photo objects with title and URLs
 */
function parseCollection() {
  console.log('📋 Parsing collection data...');
  
  const collectionContent = fs.readFileSync(CONFIG.collectionFile, 'utf-8');
  const photos = [];
  
  // Extract photo titles from the collection file
  const titleRegex = /&quot;title&quot;: &quot;([^&]+)&quot;/g;
  const titles = [];
  let matchTitle;
  
  while ((matchTitle = titleRegex.exec(collectionContent)) !== null) {
  ...
  for (const title of titles) {
    ...
    const titlePattern = new RegExp(`&quot;title&quot;: &quot;${title}&quot;[\\s\\S]+?<\\/figure>`, 'g');
    const photoChunks = collectionContent.match(titlePattern);
    
    if (photoChunks && photoChunks.length > 0) {
      // Get all image URLs from this chunk
      const urlMatches = photoChunks[0].match(/data-src="([^"]+)"/g) || [];
      const urls = urlMatches.map(url => url.replace(/^data-src="/, '').replace(/"$/, ''));
  ...
  for (const photo of photos) {
    console.log(`- "${photo.title}": ${photo.urls.length} URLs`);
  }
...
function parsePhotosFromHtml(htmlContent) {
  ...
  for (const title of titles) {
    ...
    const titlePattern = new RegExp(`"title": "${escapedTitle}"[\\s\\S]+?<\\/figure>`, 'g');
...
async function main() {
  try {
    console.log('🚀 Starting Find Variations Downloader');
    ...
    // Get collection URL or use default file
    let photos;
    if (CONFIG.collectionUrls[collectionName]) {
      console.log(`🌐 Fetching collection data from URL: ${CONFIG.collectionUrls[collectionName]}`);
      try {
        const htmlContent = await fetchHtmlContent(CONFIG.collectionUrls[collectionName]);
        photos = parsePhotosFromHtml(htmlContent);
      } catch (error) {
        console.error(`❌ Error fetching collection data: ${error.message}`);
        console.log('⚠️ Falling back to local collection file...');
        photos = parseCollection();
      }
    } else {
      console.log('📄 Using local collection file...');
      photos = parseCollection();
    }
