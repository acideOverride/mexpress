...
1. Monorepo API Structure:
   ```text
   # Core API Routes (from packages/core)
   /api/v{version}/core/{resource}/{identifier}/{sub-resource}

   Examples:
   GET    /api/v1/core/auth/login           # Authentication
   POST   /api/v1/core/users                # User management
   GET    /api/v1/core/roles                # Role management
   
   # Project-specific API Routes (from projects/mexpress/backend)
   /api/v{version}/{project}/{resource}/{identifier}/{sub-resource}

   Examples:
   GET    /api/v1/mexpress/customers        # List customers
   POST   /api/v1/mexpress/orders           # Create order
   GET    /api/v1/mexpress/products/:id     # Get product
   PUT    /api/v1/mexpress/inventory/:id    # Update inventory
   
   # Shared Utility Routes (from packages/utils)
   /api/v{version}/utils/{utility}/{action}

   Examples:
   POST   /api/v1/utils/validation/email    # Email validation
   GET    /api/v1/utils/formatting/date     # Date formatting
   ```

2. Route Organization:
   ```typescript
   // packages/core/src/routes/auth.routes.ts
   @Controller('api/v1/core/auth')
   ...
   // packages/core/src/routes/users.routes.ts
   ...
   // projects/mexpress/backend/src/routes/customers.routes.ts
   @Controller('api/v1/mexpress/customers')
   ...
   // packages/utils/src/routes/validation.routes.ts
   @Controller('api/v1/utils/validation')
...
1. Monorepo API Documentation:
   ...
   info:
     title: Core API
     version: 2.0.0
     description: Core functionality for mExpress services
   ...
   # packages/utils/openapi.yaml
   openapi: 3.0.0
   info:
     title: Utils API
     version: 2.0.0
     description: Shared utilities for mExpress services
   
   paths:
     /api/v1/utils/validation/email:
       post:
   ...
   # projects/mexpress/backend/openapi.yaml
   openapi: 3.0.0
   info:
     title: mExpress Project API
     version: 2.0.0
     description: Project-specific APIs for mExpress
   
   paths:
     /api/v1/mexpress/customers:
       get:
         summary: List customers
         tags: [mExpress - Customers]
         responses:
           200:
...
2. Documentation Organization:
   ```typescript
   // packages/core/src/docs/swagger.config.ts
   export const coreSwaggerConfig = {
     title: 'Core API',
     description: 'Core functionality documentation',
     version: '2.0.0',
   ...
   // packages/utils/src/docs/swagger.config.ts
   export const utilsSwaggerConfig = {
     title: 'Utils API',
     ...
     servers: [
       {
         url: '/api/v1/utils',
         description: 'Utils API endpoint'
       }
     ]
   };

   // projects/mexpress/backend/src/docs/swagger.config.ts
   export const projectSwaggerConfig = {
     title: 'mExpress API',
     description: 'Project-specific API documentation',
     version: '2.0.0',
     tags: ['Customers', 'Orders', 'Products'],
     servers: [
       {
         url: '/api/v1/mexpress',
...
3. Documentation Aggregation:
   ```typescript
   // scripts/docs/aggregate-api-docs.ts
   async function aggregateApiDocs() {
     // 1. Load all OpenAPI specs
     const coreSpec = await loadSpec('packages/core/openapi.yaml');
     const utilsSpec = await loadSpec('packages/utils/openapi.yaml');
     const projectSpec = await loadSpec('projects/mexpress/backend/openapi.yaml');

     // 2. Merge specs
     const combinedSpec = {
       openapi: '3.0.0',
       info: {
         title: 'mExpress Complete API',
         version: '2.0.0',
         description: 'Combined API documentation for mExpress'
       },
       paths: {
         ...coreSpec.paths,
         ...utilsSpec.paths,
       ...
       components: {
         schemas: {
           ...
           ...utilsSpec.components.schemas,
           ...projectSpec.components.schemas
         }
       }
     };

     // 3. Generate documentation
     await generateDocs(combinedSpec);
   }
   ```
