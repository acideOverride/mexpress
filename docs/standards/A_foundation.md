...
- projects/          # Project-specific data
  - {project_name}/  # Per-project storage
    - uploads/      # User uploaded files
    - temp/         # Temporary processing files
    - cache/        # Application cache
- packages/          # Package-specific data
  - core/           # Core service data
  - ui-components/  # UI component assets
  - utils/          # Utility data

#### Logs (/var/log/mexpress/)
- projects/          # Project-specific logs
  - {project_name}/  # Per-project logs
    - access.log    # HTTP access logs
    - error.log     # Application error logs
    - audit.log     # Security audit logs
    - debug.log     # Debug information
- packages/          # Package-specific logs
  - core/           # Core service logs
  - ui-components/  # UI component logs
  - utils/          # Utility logs

#### Configuration (/etc/mexpress/)
- packages/          # Package configurations
  - core/           # Core service config
    - ssl/          # SSL certificates
    - nginx/        # Nginx configuration
    - env/          # Environment configurations
  - ui-components/  # UI component config
  - utils/          # Utilities config
- projects/          # Project configurations
  - {project_name}/  # Per-project config
    - ssl/          # Project SSL certs
    - nginx/        # Project nginx config
    - env/          # Project env config
...
#### Symbolic Links
# Package Links
- packages/core/logs -> /var/log/mexpress/packages/core
- packages/core/data -> /var/lib/mexpress/packages/core
- packages/ui-components/logs -> /var/log/mexpress/packages/ui-components
- packages/ui-components/data -> /var/lib/mexpress/packages/ui-components
- packages/utils/logs -> /var/log/mexpress/packages/utils
- packages/utils/data -> /var/lib/mexpress/packages/utils

# Project Links
- projects/{project_name}/logs -> /var/log/mexpress/projects/{project_name}
- projects/{project_name}/uploads -> /var/lib/mexpress/projects/{project_name}/uploads
...
sudo mkdir -p /opt/mexpress/packages/{core,ui-components,utils}/{src,tests}
sudo mkdir -p /opt/mexpress/projects/{project_name}/{frontend,backend}
sudo mkdir -p /opt/mexpress/docs/{core,projects}

# Create data directories
sudo mkdir -p /var/lib/mexpress/packages/{core,ui-components,utils}/{data,cache}
sudo mkdir -p /var/lib/mexpress/projects/{project_name}/{uploads,temp,cache}

# Create log directories
sudo mkdir -p /var/log/mexpress/packages/{core,ui-components,utils}
sudo mkdir -p /var/log/mexpress/projects/{project_name}

# Create config directories
sudo mkdir -p /etc/mexpress/packages/{core,ui-components,utils}/{ssl,nginx,env}
sudo mkdir -p /etc/mexpress/projects/{project_name}/{ssl,nginx,env}

# Create backup directories
sudo mkdir -p /var/backups/mexpress/{packages,projects}/{daily,weekly,monthly}
...
# Set package-specific permissions
for pkg in core ui-components utils; do
    sudo chown -R $USER:$USER /opt/mexpress/packages/$pkg
    sudo chown -R www-data:www-data /var/lib/mexpress/packages/$pkg
    sudo chown -R www-data:www-data /var/log/mexpress/packages/$pkg
done

# Set project-specific permissions
for proj in $(ls /opt/mexpress/projects/); do
    sudo chown -R $USER:$USER /opt/mexpress/projects/$proj
    sudo chown -R www-data:www-data /var/lib/mexpress/projects/$proj
    sudo chown -R www-data:www-data /var/log/mexpress/projects/$proj
done

# Create symbolic links for packages
for pkg in core ui-components utils; do
    ln -s /var/log/mexpress/packages/$pkg /opt/mexpress/packages/$pkg/logs
    ln -s /var/lib/mexpress/packages/$pkg /opt/mexpress/packages/$pkg/data
done

# Create symbolic links for projects
for proj in $(ls /opt/mexpress/projects/); do
    ln -s /var/log/mexpress/projects/$proj /opt/mexpress/projects/$proj/logs
    ln -s /var/lib/mexpress/projects/$proj/uploads /opt/mexpress/projects/$proj/uploads
done
...
validate_setup() {
    ...
    # Check monorepo directory structure
    echo "Validating monorepo directory structure..."
    local required_dirs=(
        # Package directories
        "/opt/mexpress/packages/core/src"
        "/opt/mexpress/packages/core/tests"
        "/opt/mexpress/packages/ui-components/src"
        "/opt/mexpress/packages/ui-components/tests"
        "/opt/mexpress/packages/utils/src"
        "/opt/mexpress/packages/utils/tests"
        
        # Project directories
        "/opt/mexpress/projects"
        "/opt/mexpress/docs/core"
        "/opt/mexpress/docs/projects"
        
        # Data directories
        "/var/lib/mexpress/packages/core/data"
        "/var/lib/mexpress/packages/ui-components/data"
        "/var/lib/mexpress/packages/utils/data"
        "/var/lib/mexpress/projects"
        
        # Log directories
        "/var/log/mexpress/packages/core"
        "/var/log/mexpress/packages/ui-components"
        "/var/log/mexpress/packages/utils"
        "/var/log/mexpress/projects"
        
        # Config directories
        "/etc/mexpress/packages/core/env"
        "/etc/mexpress/packages/ui-components/env"
        "/etc/mexpress/packages/utils/env"
        "/etc/mexpress/projects"
        
        # Backup directories
        "/var/backups/mexpress/packages/daily"
        "/var/backups/mexpress/packages/weekly"
        "/var/backups/mexpress/packages/monthly"
        "/var/backups/mexpress/projects/daily"
        "/var/backups/mexpress/projects/weekly"
        "/var/backups/mexpress/projects/monthly"
    )
    ...
    # Check package symbolic links
    for pkg in core ui-components utils; do
        if [[ ! -L "/opt/mexpress/packages/$pkg/logs" ]] || [[ ! -e "/opt/mexpress/packages/$pkg/logs" ]]; then
            echo "Error: Invalid or missing logs symbolic link for package $pkg"
            exit_code=1
        fi
        
        if [[ ! -L "/opt/mexpress/packages/$pkg/data" ]] || [[ ! -e "/opt/mexpress/packages/$pkg/data" ]]; then
            echo "Error: Invalid or missing data symbolic link for package $pkg"
            exit_code=1
        fi
    done
    
    # Check project symbolic links
    for proj in $(ls /opt/mexpress/projects/); do
        if [[ ! -L "/opt/mexpress/projects/$proj/logs" ]] || [[ ! -e "/opt/mexpress/projects/$proj/logs" ]]; then
            echo "Error: Invalid or missing logs symbolic link for project $proj"
            exit_code=1
        fi
        
        if [[ ! -L "/opt/mexpress/projects/$proj/uploads" ]] || [[ ! -e "/opt/mexpress/projects/$proj/uploads" ]]; then
            echo "Error: Invalid or missing uploads symbolic link for project $proj"
            exit_code=1
        fi
    done
    ...
    for pkg in core ui-components utils; do
        if [[ $(stat -c '%a' "/opt/mexpress/packages/$pkg") != "755" ]]; then
            echo "Error: Incorrect permissions on package directory $pkg"
            exit_code=1
        fi
        if [[ $(stat -c '%a' "/var/log/mexpress/packages/$pkg") != "744" ]]; then
            echo "Error: Incorrect permissions on package logs directory $pkg"
            exit_code=1
        fi
    done
    ...
    for proj in $(ls /opt/mexpress/projects/); do
        if [[ $(stat -c '%a' "/opt/mexpress/projects/$proj") != "755" ]]; then
            echo "Error: Incorrect permissions on project directory $proj"
            exit_code=1
        fi
        if [[ $(stat -c '%a' "/var/log/mexpress/projects/$proj") != "744" ]]; then
            echo "Error: Incorrect permissions on project logs directory $proj"
            exit_code=1
        fi
        if [[ $(stat -c '%a' "/var/lib/mexpress/projects/$proj/uploads") != "755" ]]; then
            echo "Error: Incorrect permissions on project uploads directory $proj"
            exit_code=1
        fi
    done
