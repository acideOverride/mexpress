...
1. Service Organization in Monorepo:
   - Core services in core package
   - Project-specific services in project
   - Service composition and inheritance
   - Dependency injection patterns
   - Cross-package service integration

```typescript
// packages/core/src/services/auth/base-auth.service.ts
@Injectable()
export class BaseAuthService {
  ...
  protected async validateCredentials(email: string, password: string): Promise<User> {
...
// packages/core/src/services/auth/auth.service.ts
@Injectable()
...
// projects/mexpress/backend/src/services/custom-auth.service.ts
@Injectable()
export class CustomAuthService extends BaseAuthService {
  constructor(
    ...
    super(userRepository, eventEmitter, encryptionService);
  }

  async login(credentials: CustomLoginDto): Promise<CustomAuthResponse> {
    this.customLogger.log('Login attempt', credentials.email);
    ...
    // Custom project-specific login logic
    const customToken = await this.generateCustomToken(user);
    this.eventEmitter.emit('custom.login', { userId: user.id });
    
    return {
      user,
      token: customToken,
      customData: await this.getCustomUserData(user)
    };
  }
}

// Example usage in project controller
@Controller('auth')
export class AuthController {
  ...
  @Post('login')
  async login(@Body() credentials: CustomLoginDto): Promise<CustomAuthResponse> {
...
1. Repository Organization in Monorepo:
   - Core repositories in core package
   - Project-specific repositories
   - Repository inheritance patterns
   - Shared caching strategies
   - Cross-package repository usage

```typescript
// packages/core/src/repositories/base/base.repository.ts
@Injectable()
export abstract class BaseRepository<T extends BaseEntity> {
  ...
  protected async getCached<R>(
    ...
    const cached = await this.cacheManager.get<R>(key);
...
// packages/core/src/repositories/user/user.repository.ts
@Injectable()
export class CoreUserRepository extends BaseRepository<User> {
...
// projects/mexpress/backend/src/repositories/custom-user.repository.ts
@Injectable()
export class CustomUserRepository extends CoreUserRepository {
  async findWithProjectData(id: string): Promise<UserWithProjectData | null> {
    return this.getCached(
      `user:project:${id}`,
      async () => {
        const user = await this.findWithRoles(id);
        if (!user) return null;

        // Add project-specific data
        return {
          ...user,
          projectData: await this.getProjectData(id)
        };
      }
    );
  }

  private async getProjectData(userId: string): Promise<ProjectData> {
    return this.getCached(
      `user:projectdata:${userId}`,
      () => this.repository.createQueryBuilder('user')
        .leftJoinAndSelect('user.projectData', 'projectData')
...
// Example usage in project service
@Injectable()
export class ProjectUserService {
  ...
  async getUserWithProjectData(id: string): Promise<UserWithProjectData> {
    const user = await this.userRepository.findWithProjectData(id);
    if (!user) {
...
Example Usage:
```typescript
// Using core models and services
import { User } from '@mexpress/core/models/auth';
import { AuthService } from '@mexpress/core/services/auth';

// Using shared utilities
import { validateEmail } from '@mexpress/utils/validation';
import { formatDate } from '@mexpress/utils/formatting';

// Project-specific controller using shared components
@Controller('users')
export class UserController {
  constructor(
