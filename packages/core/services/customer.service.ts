...
export class CustomerService {
    ...
    async searchCustomers(criteria: SearchCriteria): Promise<Customer[]> {
        ...
        return this.customers.filter(customer => {
            ...
            if (criteria.name && 
                (customer.firstName.includes(criteria.name) || 
                 customer.lastName.includes(criteria.name))) {
                return true;
            }
            if (criteria.phone && customer.phone === criteria.phone) {
                return true;
            }
            return false;
        });
